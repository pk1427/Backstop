...
export default function OverviewPage() {
  ...
  const controlledStrategy = backstop.strategy?.tokenOut.toLowerCase() === '0xfd080b70baefd6bb19906c107a7240c4e5c2dcca';
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          {!backstop.authenticated ? (
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold tracking-[.16em] text-cyan-300">{controlled ? 'CONTROLLED MARKET MAKER' : 'CAPITAL MODE'}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Your authorized capital</h2><p className="mt-2 text-sm text-zinc-500">{controlled ? 'Controlled btUSDC made available by this Privy wallet for CRE-verified settlements.' : 'A live view of the limits protecting your self-custodied USDC.'}</p></div><Link href="/strategy" className="rounded-xl border border-cyan-400/40 bg-cyan-400/10 px-4 py-2.5 text-sm font-semibold text-cyan-300 transition hover:bg-cyan-400/20">Manage strategy</Link></div>
              <div className="grid divide-y divide-[#233552] rounded-2xl border border-[#233552] bg-[#070e20]/80 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4"><StatCard label={controlled ? 'Wallet btUSDC' : 'Wallet USDC'} value={backstop.usdcBalance ? `${Number(backstop.usdcBalance).toLocaleString()} ${controlled ? 'btUSDC' : 'USDC'}` : '—'} /><StatCard label="Authorized capital" value={backstop.strategy ? `${backstop.strategy.maxTrade} ${controlledStrategy ? 'btUSDC' : 'USDC'}` : '0'} /><StatCard label="Strategy status" value={controlledStrategy ? 'Controlled active' : backstop.strategy ? 'Active' : 'Setup'} /><StatCard label="Eligible now" value={backstop.latestQuote?.healthFactor && backstop.latestQuote.healthFactor < 1 ? '1' : '0'} subtext="liquidations" /></div>
              ...
              <section><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-semibold tracking-[.14em] text-cyan-300">{controlled ? 'CONTROLLED MARKET' : 'LIVE AAVE MARKET'}</p><h3 className="mt-1 text-lg font-semibold text-zinc-900 dark:text-zinc-100">{controlled ? 'Controlled liquidation monitoring' : 'Live Aave monitoring'}</h3></div><Link href="/opportunities" className="text-sm font-medium text-cyan-300 hover:text-cyan-200">Open market →</Link></div>{backstop.latestQuote ? <><OpportunityCard quote={backstop.latestQuote} loading={backstop.quoteLoading} /><Link href="/opportunities/opportunity" className="mt-4 inline-flex text-sm font-semibold text-cyan-300 hover:text-cyan-200">Review opportunity →</Link></> : <div className="protocol-card rounded-2xl p-6"><p className="font-medium text-zinc-100">No executable quote available</p><p className="mt-2 text-sm text-slate-400">{controlled ? 'Select a controlled position to inspect its borrower-bound CRE quote.' : 'A quote is created only when the selected Aave position is liquidatable.'}</p></div>}</section>

        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          {!backstop.authenticated ? (
            ...
            <div className="flex flex-col gap-8">
              ...
              {backstop.mode === 'controlled' && <ControlledMarketPositions backstop={backstop} position={backstop.aavePosition} loading={backstop.quoteLoading} />}

              {backstop.mode === 'live' ? <LivePosition position={backstop.aavePosition} loading={backstop.liveOpportunityLoading} /> : backstop.controlledPositionSettled ? null : backstop.controlledQuoteMatchesSelection ? <><OpportunityCard quote={backstop.latestQuote} loading={backstop.quoteLoading} /><div className="flex justify-end"><Link href="/opportunities/opportunity" className="rounded-xl border border-cyan-400/40 bg-cyan-400/10 px-4 py-2.5 text-sm font-semibold text-cyan-300 hover:bg-cyan-400/20">Review CRE quote →</Link></div></> : null}
...
const CONTROLLED_POSITIONS = [
  {label: 'Position 02', address: '0x85D737640a6b86EBfd1AcEB9BE9992c90020bA1c', collateral: '0.7000', debt: '1,000.00', health: '0.893', monitored: true},
  {label: 'Position 03', address: '0xf62c155Eb012303Cbba80cb246De20E05dd57051', settled: true},
  {label: 'Position 04', address: '0x0Bc87b3FBbCBEb04595A503fbCe1d627cEd222ad', collateral: '0.8000', debt: '1,200.00', health: '0.850'},
];

function ControlledMarketPositions({backstop, position, loading}: {backstop: ReturnType<typeof useBackstop>; position: ReturnType<typeof useBackstop>['aavePosition']; loading: boolean}) {
  ...
  const selectedIsHistoricSettlement = position.borrower.toLowerCase() === CONTROLLED_POSITIONS[1].address.toLowerCase();
  const selectedSettled = backstop.controlledPositionSettled || selectedIsHistoricSettlement;
  return <section><div className="mb-4"><p className="text-xs font-semibold tracking-[.14em] text-cyan-300">CONTROLLED POSITIONS</p><p className="mt-2 text-sm text-slate-400">Select a borrower to inspect its live onchain state.</p></div><div className="grid gap-4 md:grid-cols-3">{CONTROLLED_POSITIONS.map((item) => { const selected = backstop.controlledBorrowerAddress.toLowerCase() === item.address.toLowerCase(); const settled = item.monitored || item.settled; const settlementTx = item.monitored ? 'https://sepolia.etherscan.io/tx/0xfb8b9e3e545fca0eb35f9c00166fbad4ecfec4842730cf39cbcf04b33f79f272' : 'https://sepolia.etherscan.io/tx/0xa325d92a25340adf68c18f3951173e539aca8c84e26ba7c11b1f0127cb684943'; return <article key={item.address} className={`protocol-card rounded-2xl border p-5 ${selected ? 'border-cyan-400/60 bg-cyan-400/5' : 'border-[#243a5a]'}`}><div><p className="text-sm font-semibold text-white">{item.label}</p><p className={`mt-1 text-xs font-semibold ${settled ? 'text-emerald-300' : 'text-amber-300'}`}>{settled ? 'SETTLEMENT CONFIRMED' : 'AVAILABLE FOR MONITORING'}</p></div><p className="mt-3 font-mono text-xs text-slate-400">{item.address.slice(0, 10)}…{item.address.slice(-8)}</p><p className="mt-4 min-h-9 text-xs text-slate-500">{item.monitored ? '500 btUSDC repaid · 0.3500 btWETH delivered to the Privy maker.' : item.settled ? 'Earlier controlled settlement. View the confirmed transaction.' : 'Onchain position; CRE monitoring has not been assigned yet.'}</p><div className="mt-4 flex items-center justify-between"><button onClick={() => backstop.setControlledBorrowerAddress(item.address)} className="text-xs font-semibold text-cyan-300 hover:text-cyan-200">{selected ? 'Selected' : 'Inspect'} →</button><a href={settled ? settlementTx : `https://sepolia.etherscan.io/address/${item.address}`} target="_blank" rel="noreferrer" className="text-xs font-medium text-slate-400 hover:text-white">{settled ? 'Settlement ↗' : 'Explorer ↗'}</a></div></article>})}</div><article className="mt-5 protocol-card rounded-2xl border border-cyan-400/60 bg-cyan-400/5 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold text-white">Selected borrower</p><p className="mt-2 font-mono text-xs text-slate-400">{position.borrower}</p></div><div className="text-right"><p className={`text-2xl font-semibold ${liquidatable ? 'text-red-300' : 'text-cyan-300'}`}>{position.healthFactor.toFixed(3)}</p><p className={`mt-1 text-xs font-semibold ${liquidatable ? 'text-red-300' : 'text-cyan-300'}`}>{liquidatable ? 'LIQUIDATABLE' : 'HEALTHY'}</p></div></div><div className="mt-5 grid grid-cols-3 gap-4 border-t border-[#243a5a] pt-5"><Metric label="Health factor" value={position.healthFactor.toFixed(3)} /><Metric label="Collateral" value={`${position.collateralAmountFormatted} btWETH`} /><Metric label="Debt" value={`${position.debtAmountFormatted} btUSDC`} /></div><p className={`mt-4 text-xs ${selectedSettled ? 'text-emerald-300' : 'text-amber-300'}`}>{selectedSettled ? 'Settlement complete. The original 500 btUSDC CRE quote was consumed onchain; this remaining debt requires a new, smaller quote before it can be settled.' : 'This borrower is available for monitoring. It needs its own CRE quote before it can be executed.'}</p></article></section>;
}
